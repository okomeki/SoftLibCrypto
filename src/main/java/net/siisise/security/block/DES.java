...
//        cd[0] = 0; cd[1] = 0; // 念入りな掃除?
     ...
     * シフト量をあらかじめ計算したもの.
     */
    static final int[] shifts = {1, 2, 4, 6, 8, 10, 12, 14, 15, 17, 19, 21, 23, 25, 27, 0};
...
//        return e;
     ...
     */
    private final int f(int r, int kn) {
     ...
     * @return
     */
    static final int[] ip(byte[] src, int offset) {
        int[] lr = new int[2];
        for (int n = 0; n < 8; n++) {
            int n4 = n/4;
            for (int i = 7; i >= 0; i--) {
     ...
     * @return
     */
    static final byte[] ip_1(int[] src) {
        byte[] x = new byte[8];
