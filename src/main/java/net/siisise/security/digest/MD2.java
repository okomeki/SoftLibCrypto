...
public class MD2 extends BlockMessageDigest {
    ...
    public void blockWrite(byte[] input, int offset, int len) {
        ...
        // Step 4.
        System.arraycopy(input, offset, x, 16, 16);
        for (int j = 0; j < 16; j++) {
            // Step 2.
            lc = x[16 + j];
            c[j] ^= S[(lc ^ l) & 0xff]; // RFC 1319 Errata
            l = c[j];
    ...
    protected byte[] engineDigest() {
        ...
        engineUpdate(padding, 0, p); // ここまでStep 2. と Step 4. 並走
        // Step 2.
        engineUpdate(c, 0, 16); // Step 2を入力にまわして Step 4. のみ拾う

        byte[] ret = new byte[16];
        System.arraycopy(x, 0, ret, 0, 16);
        engineReset();
        return ret;
    }
